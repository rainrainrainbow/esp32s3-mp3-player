 ...
 * @version V4.7 
 * @update_log
 * 1. [重构] 任务表拆分为task_actions1和task_actions2，支持独立任务序列。
 * 2. [新增] 添加I2C命令轮询机制，接收ESP32-S3触控按钮指令。
 * 3. [新增] 命令码：0xA1=执行任务表1，0xA2=执行任务表2，0xAB=中止任务返回APP模式。
 * 4. [保留] 硬件PWM输出与找线防越线刹车机制。
 */
...
#define NOTONE 0
typedef enum {
  MODE_NONE, MODE_ROCKERANDGRAVITY, MODE_RGB_ADJUST,
  MODE_SPEED_CONTROL, MODE_ULTRASOUND_SEND, MODE_VOLTAGE_SEND, MODE_AVOID
} CarMode;

typedef enum { APP_MODE, TRACKING_MODE } RunMode;
typedef enum { WARNING_OFF, WARNING_BEEP, WARNING_RGB } VoltageWarning;
typedef enum {
  STATE_TRACKING, STATE_CROSS_OVER, STATE_TASK_HANDLE, STATE_CORNER_TURNING
} TrackingState;
...
void Parse_Serial_NonBlocking(void) {
  ...
  while (Serial.available()) {
    ...
    if (c == '$') {
      ...
      if (rec_data[0] == "A" && avoid_flag == 0) {
        g_runMode = APP_MODE; g_mode = MODE_ROCKERANDGRAVITY;
        ...
        g_mode = MODE_AVOID; avoid_flag = 1; g_state = rec_data[1].toInt();
      }
      cmdBuffer = "";
    } else {
      cmdBuffer += c;
...
void Task_Handle_Task(void) {
  ...
  if (shouldExit) {
    bool is_find_line_action = (action_code == ACT_ALIGN_L || action_code == ACT_ALIGN_R ||
                                action_code == ACT_SHIFT_L || action_code == ACT_SHIFT_R);
